import torch
import torch.nn as nn
import torch.nn.functional as F
import math

class AttentionBlock(nn.Module):
    """
    Standard Multihead Attention Block (MAB).
    Computes attention between Query (Q) and Key/Value (K).
    """
    def __init__(self, dim_Q, dim_K, dim_V, num_heads, ln=False, dropout=0.0):
        super(AttentionBlock, self).__init__()
        self.dim_V = dim_V
        self.num_heads = num_heads
        
        # Calculate dimension per head for scaling
        self.dim_split = self.dim_V // self.num_heads
        
        self.attention_dropout = nn.Dropout(p=dropout)
        self.residual_dropout = nn.Dropout(p=dropout)

        self.fc_q = nn.Linear(dim_Q, dim_V)
        self.fc_k = nn.Linear(dim_K, dim_V)
        self.fc_v = nn.Linear(dim_K, dim_V)
        
        if ln:
            self.ln0 = nn.LayerNorm(dim_V)
            self.ln1 = nn.LayerNorm(dim_V)
        
        self.fc_o = nn.Linear(dim_V, dim_V)

    def forward(self, Q, K, return_attention=False):
        # Linear projections
        Q_proj = self.fc_q(Q)
        K_proj, V_proj = self.fc_k(K), self.fc_v(K)

        # Split into heads and concat: (Batch, Num_Heads * Seq_Len, Dim_Head)
        # Note: dim_split is used to split the last dimension
        Q_ = torch.cat(Q_proj.split(self.dim_split, 2), 0)
        K_ = torch.cat(K_proj.split(self.dim_split, 2), 0)
        V_ = torch.cat(V_proj.split(self.dim_split, 2), 0)

        # Scaled Dot-Product Attention
        # CRITICAL FIX: Scale by sqrt(dim_head), not sqrt(dim_total)
        scale_factor = math.sqrt(self.dim_split)
        score = Q_.bmm(K_.transpose(1, 2)) / scale_factor
        
        A = torch.softmax(score, 2)
        A_do = self.attention_dropout(A)

        # Apply attention to V and concatenate heads back
        O = torch.cat((Q_ + A_do.bmm(V_)).split(Q.size(0), 0), 2)
        
        # Residual connection + Layer Norm (if enabled)
        O = O if getattr(self, 'ln0', None) is None else self.ln0(O)
        
        # Feed-forward block
        O = O + self.residual_dropout(F.relu(self.fc_o(O)))
        O = O if getattr(self, 'ln1', None) is None else self.ln1(O)

        if return_attention:
            # Reshape A back to (Batch, Heads, Q_len, K_len) and take mean across heads for visualization
            A_reshaped = A.view(-1, self.num_heads, A.size(1), A.size(2))
            A_mean = A_reshaped.mean(dim=1)
            return O, A_mean
            
        return O

class MoleculeSimulator(nn.Module):
    """
    Self-Attention Block (SAB).
    Simulates interactions between elements in the set (e.g., Molecule A <-> Molecule B).
    """
    def __init__(self, dim_in, dim_out, num_heads, ln=False, dropout=0.0):
        super(MoleculeSimulator, self).__init__()
        self.mab = AttentionBlock(dim_in, dim_in, dim_out, num_heads, ln=ln, dropout=dropout)

    def forward(self, X, return_attention=False):
        return self.mab(X, X, return_attention=return_attention)

class CrossIntegration(nn.Module):
    """
    Pooling Multihead Attention (PMA).
    Integrates variable-sized set information into fixed-size Seed Vectors via Cross-Attention.
    """
    def __init__(self, dim, num_heads, num_seeds, ln=False, dropout=0.0):
        super(CrossIntegration, self).__init__()
        self.S = nn.Parameter(torch.Tensor(1, num_seeds, dim))
        nn.init.xavier_uniform_(self.S)
        self.mab = AttentionBlock(dim, dim, dim, num_heads, ln=ln, dropout=dropout)

    def forward(self, X):
        return self.mab(self.S.repeat(X.size(0), 1, 1), X)
